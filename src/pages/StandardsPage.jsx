...
export default function StandardsPage() {
  ...
  const handleExtractContent = async () => {
    ...
    try {
      ...
      if (res.success) {
        ...
        showNotice("Extraction Error", res.message || "Failed to extract content from files.", "error");
      }
    } catch (err) {
      console.error("Extraction error:", err);
      showNotice("Extraction Error", "Failed to extract content from files.", "error");
    } finally {
  ...
  const confirmDeleteStandard = async () => {
    ...
    setIsDeletingStandard(true);
    try {
      await deleteStandard(deleteConfirmTarget.id);
      setDeleteConfirmTarget(null);
      fetchStandards(true, activeTab);
    } catch (err) {
      console.error("Delete error:", err);
      showNotice("Delete Failed", "Failed to delete file.", "error");
    } finally {
      setIsDeletingStandard(false);
    }
  };
  ...
  return (
    <div className="animate-fade-in-up flex flex-col h-full">
                ...
                <div className="flex flex-col h-full overflow-hidden">
                  <div className="flex justify-between items-center mb-6 pb-4 border-b">
                    ...
                    <div className="flex gap-2">
                      ...
                      <button onClick={() => setDeleteConfirmTarget(selectedStandard)} className="p-1.5 border border-red-200 bg-red-50 text-red-600 rounded hover:bg-red-100 transition-colors" title="Delete file">
      ...
      {/* Custom UI Confirmation Modal (For Discard Changes) */}
      {confirmModal && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4 transition-all animate-fade-in">
          <div className="bg-white rounded-2xl max-w-md w-full p-6 shadow-2xl border border-gray-100 flex flex-col items-center text-center transform transition-all scale-100">
      ...
      {deleteConfirmTarget && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4 transition-all animate-fade-in">
          <div className="bg-white rounded-2xl max-w-md w-full p-6 shadow-2xl border border-gray-100 flex flex-col items-center text-center transform transition-all scale-100">
            <div className="w-14 h-14 rounded-2xl bg-red-50 text-red-500 border border-red-100 flex items-center justify-center mb-4 text-2xl font-bold shadow-sm">
              🗑️
            </div>
            <h3 className="text-base font-extrabold text-sidebar mb-1.5">Confirm Deletion</h3>
            <p className="text-xs text-gray-600 mb-6 leading-relaxed font-medium">
              Are you sure you want to delete <strong className="text-sidebar font-bold">"{deleteConfirmTarget.filename}"</strong>?
              <br />
              <span className="text-red-500 text-[11px] block mt-1">This action cannot be undone.</span>
            </p>
            <div className="flex gap-3 w-full">
              <button 
                onClick={() => setDeleteConfirmTarget(null)}
                disabled={isDeletingStandard}
              ...
              <button 
                ...
                disabled={isDeletingStandard}
                className="flex-1 py-2.5 bg-red-600 text-white rounded-xl text-xs font-bold hover:bg-red-700 transition-colors shadow-md shadow-red-500/20 flex items-center justify-center gap-1.5 disabled:opacity-50"
              >
                {isDeletingStandard ? (
                  <>
                    <svg className="animate-spin h-3.5 w-3.5 text-white" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
